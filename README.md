...
YiFPGA Studio 是面向 FPGA 的开放调试与可观测性平台。v1.0.0 以 ATK-KU5 上的 Xilinx
`xcku5p-ffvb676-2-i` 为已验证参考平台，提供 Debug Core、Trace、Monitor、Profiler、
Logic Analyzer、AI Debug，以及 UART/JTAG 数据通道和统一 Web Viewer。
...
- Transport：UART和Xilinx BSCAN/USER2 JTAG；normal 和 performance+ILA 板级闭环已通过，performance 30 分钟长稳达到 154,193.906 B/s。
...
当前源码为独立仓库的 **v1.0.0 正式发布版本**。仓库只包含 YiFPGA canonical 入口，
不携带旧品牌兼容层或迁移历史。

发布参考基线为 ATK-KU5、`xcku5p-ffvb676-2-i`、Vivado 2024.2 Build 5239630、UART
`115200 8N1` 和 Digilent JTAG cable。performance+ILA 镜像的 30 分钟实测平均吞吐为
154,193.906 B/s，验证窗口内 drop/overflow 均为 0。
...
- Node.js 20
- just 1.56
- Vivado 2024.2 Build 5239630（仅 FPGA elaboration、综合、实现、bitstream 和烧录需要）
...
- [v1.0.0 Release Notes](doc/RELEASE_NOTES_v1.0.0.md)
