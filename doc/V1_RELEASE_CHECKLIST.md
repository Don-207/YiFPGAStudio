...
- [ ] 记录 Python、Node.js 和 just 版本。
- [ ] 记录 Vivado 完整版本及补丁级别；当前基线为 2024.2。
- [ ] 确认目标器件为 `xcku5p-ffvb676-2-i`。
- [ ] 复核 `prj/constraints/yifpga_debug_board_demo.xdc` 与实际板卡引脚、电平和时钟一致。
- [ ] 记录板卡编号、JTAG cable 精确 target 名称和串口设备路径。
...
- [ ] `manifest.txt` 中器件、功能开关和 BSCANE2 数量符合预期。
- [ ] `utilization.rpt` 无异常资源增长。
- [ ] `clock_interaction.rpt` 和 `cdc.rpt` 已审查且风险可接受。
...
- [ ] `manifest.txt` 中 `wns_ns` 非负，BSCANE2 和 ILA 数量均为 1。
- [ ] 无未布线网络。
- [ ] Timing Summary 满足时序要求。
- [ ] DRC、CDC 和资源报告已人工审查。
- [ ] 记录 `.bit`、`.ltx` 和发布提交的 SHA-256。
...
- [ ] 命令只匹配一个 target 和一个 FPGA。
- [ ] 烧录成功，且恰好枚举一个 ILA。
- [ ] UART 在 `115200 8N1` 下持续输出有效 Debug Protocol 帧。
- [ ] Web Viewer 的 Debug、Trace、Monitor、Profiler、Logic Analyzer 和 AI Debug 基本流程通过。
- [ ] Monitor 写操作只使用可恢复测试值，并确认恢复原值。
- [ ] 断开/重连 JTAG Bridge 后数据流恢复，无新增 drop/overflow。

启动已配置的本地 JTAG Bridge 后，另开终端执行 30 分钟 soak：
...
- [ ] 平均吞吐不低于 100,000 B/s。
- [ ] `last_error` 为空。
- [ ] drop/overflow 计数在验证窗口内不增长。
- [ ] 三次客户端重连均成功。
- [ ] 保存 soak CSV、Bridge 日志和异常说明。

## 6. 发布

- [ ] 将 `CHANGELOG.md` 的 `Unreleased` 内容转为 `1.0.0` 并填写发布日期。
- [ ] README 不再将版本描述为候选版本。
